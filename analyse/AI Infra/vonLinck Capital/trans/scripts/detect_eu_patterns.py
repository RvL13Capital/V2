"""
Detect Patterns in Expanded EU Universe (Optimized)
===================================================

Runs pattern detection on all EU tickers in the local data cache.
...
Usage:
    python scripts/detect_eu_patterns.py --output output/eu_patterns_expanded.parquet
    python scripts/detect_eu_patterns.py --workers 4
"""
...
def get_eu_tickers_from_cache(data_dir: Path) -> list:
    """Get all EU tickers from local cache."""
    tickers = []
    # Check Parquet
    for f in data_dir.glob('*.parquet'):
        if any(f.stem.endswith(suffix) for suffix in EU_SUFFIXES):
            tickers.append(f.stem)
    # Check CSV
    for f in data_dir.glob('*.csv'):
        if f.stem not in tickers and any(f.stem.endswith(suffix) for suffix in EU_SUFFIXES):
            tickers.append(f.stem)
...
def main():
    parser = argparse.ArgumentParser(description='Detect patterns in expanded EU universe')
    parser.add_argument('--output', type=str, default='output/eu_patterns_expanded.parquet')
    ...
    # Init Cache
    tickers = get_eu_tickers_from_cache(data_dir)
    if args.limit:
    ...
    # Clean Chunks Dir
    chunks_dir = output_path.parent / "chunks_temp"
    if chunks_dir.exists():
        shutil.rmtree(chunks_dir)
    chunks_dir.mkdir(parents=True, exist_ok=True)

    # Stats Accumulator (Track without holding DF)
    stats = {'total': 0, 'markets': {}, 'classes': {}}
        ...
        if patterns:
            ...
            for p in patterns:
                ...
                for s in EU_SUFFIXES:
                    if p['ticker'].endswith(s):
                        stats['markets'][s] = stats['markets'].get(s, 0) + 1
                        break
