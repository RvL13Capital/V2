"""
TRAnS V21 "Phase-State"
=======================
Production Architecture.
Topology: CNN(Geometry) -> RoPE -> PhaseTransitionPooling -> FiLM(Regime) -> MLP.
Key Innovation: Explicitly models State Transition (Delta) between Setup and Trigger phases.
"""
...
# -----------------------------------------------------------------------------
# 3. Phase-Transition Pooling (The State Detector)
# -----------------------------------------------------------------------------
class PhaseTransitionPooling(nn.Module):
    """
    Explicitly models the transition from Setup to Trigger.
    Output: [Setup, Trigger, Delta]
    """
    def __init__(self, input_dim: int, split_idx: int = 16):
        super().__init__()
        self.split_idx = split_idx
        # Output dim = 3x input because we concat Setup, Trigger, and Delta
        self.output_dim = input_dim * 3 

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, T, D)
        # Setup: 0 to split_idx (The Coil)
        setup = x[:, :self.split_idx, :].mean(dim=1) 
        
        # Trigger: split_idx to end (The Ignition)
        trigger = x[:, self.split_idx:, :].mean(dim=1)
        
        # Delta: The Explicit State Change
        delta = trigger - setup
        
        return torch.cat([setup, trigger, delta], dim=1)

# -----------------------------------------------------------------------------
# 4. Regime Modulation (GRN + FiLM)
# -----------------------------------------------------------------------------
...
class HybridFeatureNetworkV21(nn.Module):
    def __init__(
        ...
        # Trigger phase length (last 4 days for T=20)
        self.split_idx = max(1, sequence_length - 4)
        ...
        # Phase-Transition Pooling
        self.phase_pool = PhaseTransitionPooling(self.cnn_dim, self.split_idx)
        self.state_dim = self.cnn_dim * 3 # [Setup, Trigger, Delta]
        ...
        # Fusion: FiLM
        # Modulates the Phase-State Vector (3 * cnn_dim)
        self.film = FiLMGenerator(context_dim=context_hidden, feature_dim=self.state_dim)

        # Decision Layer
        self.classifier = nn.Sequential(
            nn.Linear(self.state_dim, fusion_hidden),
    ...
    def forward(self, x: torch.Tensor, context: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        attn_out = self.structure_norm(attn_out)
        
        # Explicit Phase Modeling
        state_vec = self.phase_pool(attn_out) # (B, 288)

        # Branch B: Regime
        ctx_proj = F.relu(self.context_projection(context))
        context_emb = self.context_grn(ctx_proj)

        # FiLM Modulation (Regime gates the State Transition)
        gamma, beta = self.film(context_emb)
        modulated_state = (state_vec * gamma) + beta

        # Decision
        return self.classifier(modulated_state)
