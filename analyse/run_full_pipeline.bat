@echo off
REM Full Production Pipeline - Extract features from ALL GCS tickers
REM Target: 10,000+ patterns from 3,548+ tickers

echo ======================================================================
echo FULL PRODUCTION PIPELINE - GCS Dataset Processing
echo ======================================================================
echo.
echo This will process ALL tickers from GCS (3,548+) and may take 4-8 hours
echo.
...
echo.
echo ======================================================================
echo STEP 1: Pattern Detection + Feature Extraction (All GCS Tickers)
echo ======================================================================
echo This will:
echo   - Scan all 3,548+ tickers from GCS
echo   - Detect consolidation patterns (target: 10,000+)
echo   - Extract 28+ volume features per pattern
echo   - Save to AI Infra\data\features\full_pattern_features_*.parquet
echo.
echo Estimated time: 4-6 hours
echo.

python extract_all_features.py --min-patterns 10000
if errorlevel 1 (
    echo.
    echo ======================================================================
    echo X PRODUCTION FAILED - Feature extraction failed
    echo ======================================================================
    echo Check log file: extract_features_*.log
    pause
    exit /b 1
)
...
REM Find the most recent features file
...
REM Create temporary script to prepare production training data
...
echo.
echo Output files:
echo   - Patterns: output\patterns_full_gcs_*.parquet
echo   - Features: AI Infra\data\features\full_pattern_features_*.parquet
...
echo   - Log:      extract_features_*.log
echo.
echo Next steps:
echo   1. Review production_training_metadata.json for dataset stats
